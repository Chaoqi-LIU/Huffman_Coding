...
pub mod map {
    ...
    use super::mappers::char_count_mapper;
    ...
    pub fn split_into_chunks(items: String, num_chunks: usize) -> Vec<String> {
        ...
        for i in 0..num_chunks { res.push(String::new()); }
        for i in 0..items.len() {  res[i % num_chunks].push_str(items[i]);  }
        return res;
    }

    pub fn multi_threaded_mapper(words: String, num_chunks: usize) 
                    -> (Vec<JoinHandle<()>>, Receiver<HashMap<char, i32>>) {
        ...
        for i in split_into_chunks(words, num_chunks) {
            ...
            ret.push(thread::spawn(move || {
                tx_clone.send(char_count_mapper(i)).unwrap();
            }));
        }
        return (ret, rx);
    }
